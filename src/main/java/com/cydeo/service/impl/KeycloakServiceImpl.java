...
public class KeycloakServiceImpl implements KeycloakService {
    ...
    public Response userCreate(UserDTO userDTO) {
        ...
        CredentialRepresentation credential = new CredentialRepresentation();//is coming from keycloak dependency.
        credential.setType(CredentialRepresentation.PASSWORD);
        credential.setTemporary(false);
        credential.setValue(userDTO.getPassWord()); //we capture password from postman/Api and assign UserPassword in the KeyCloak;

        UserRepresentation keycloakUser = new UserRepresentation(); //This is all the field in the KeyCloak when creating user.
        ...
        Keycloak keycloak = getKeycloakInstance();//its ready code, creating instance from keycloak. We just should remember that we need instance.
        ...
        RoleRepresentation userClientRole = realmResource.clients().get(appClient.getId()) //It is going to keycloak, checks all the roles and then taking userDto and matches the role in the keycloak. if not match it will throw error.
